...
// 这是售货用的串口通讯类-货到测试、出货用
public class com_zhongji_aisle_test
{
	...
	private static com_zhongji_aisle_test ins;
	private static Context mContext;
	public static com_zhongji_aisle_test getInstance(String serilno, Context context){
		mContext = context;
		configLog();
		if(ins==null){
			synchronized (com_zhongji_aisle_test.class){
				if(ins==null){
					ins = new com_zhongji_aisle_test(serilno);
				}
			}
		}
		return ins;
	...
	private com_zhongji_aisle_test(String serialno){
		//(1) 串口定义
		serialPort = new SodoSerialPort();
		boolean open = serialPort.open(serialno,//串口号，dev/tty已经默认添加直接写例如S1，S2之类的串口号
				9600,//波特率
				8, 1, 'n');
		if(!open){
			TcnUtility.getToast(mContext,"没有开启串口权限");
			return;
		}
...
//				return recStr+"--->"+tmp;
	...
	public String checkChuHuo(int cmd){
		if ((null == outputStream) || (0 == cmd)) {
			return null;
		}
		try{
			String CMD = null;
			...
			String sendStr = "01fe"+CMD+JIAN+"aa55";
	...
	public String checkChuHuo1(String sendStr){
		if ((null == outputStream) || (null == sendStr)) {
			return null;
		}
		try{
			Log.d("TAG", "出货:"+ sendStr);
			gLogger.debug("出货:"+ sendStr);
			rxByteArray = null;
			byte[] sendCmd = TransformUtils.HexString2Bytes(sendStr);
			outputStream.write(sendCmd);
			outputStream.flush();
	...
	public String checkDoor(String cmd){
		if ((null == outputStream) || (cmd== null)) {
			return null;
		}
		try{
			Log.d("TAG", "出货:"+ cmd);
			rxByteArray = null;
			byte[] sendCmd = TransformUtils.HexString2Bytes(cmd);
			outputStream.write(sendCmd);
			outputStream.flush();
